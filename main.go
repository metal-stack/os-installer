...
import (
	...
	os "github.com/metal-stack/os-installer/pkg/installer"
	"github.com/stretchr/testify/assert/yaml"
)

func main() {
	log := slog.New(slog.NewJSONHandler(goos.Stdout, &slog.HandlerOptions{Level: slog.LevelDebug}))

	data, err := goos.ReadFile(v1.MachineDetailsPath)
	if err != nil {
	...
	data, err = goos.ReadFile(v1.MachineAllocationPath)
	if err != nil {
	...
	if err = yaml.Unmarshal(data, &allocation); err != nil {
		log.Error("unable to parse machine allocation", "error", err)
		goos.Exit(1)
	}

	if err := os.Install(context.Background(), log, &details, &allocation); err != nil {
