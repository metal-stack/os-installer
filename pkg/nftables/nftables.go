...
func Render(ctx context.Context, cfg *Config) (changed bool, err error) {
	cfg.Log.Debug("render nftables configuration")
...
func getSNAT(cfg *Config) ([]snat, error) {
	...
	for _, n := range cfg.Network.AllocationNetworks() {
		...
		cfg.Log.Debug("getSNAT", "network", n.Network)
		var (
			sources []addrSpec
		...
		for _, pfx := range privatePrimaryPrefixes {
			...
			cfg.Log.Debug("getSNAT", "network", n.Network, "prefixes", pfx, "af", af)
		}
...
func validate(cfg *Config, path string) error {
	cfg.Log.Debug("running 'nft --check --file' to validate changes.", "file", path)
