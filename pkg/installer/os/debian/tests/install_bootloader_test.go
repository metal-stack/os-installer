...
func Test_os_GrubInstall(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			execMocks: []test.FakeExecParams{
				{
					WantCmd:  []string{"grub-install", "--target=x86_64-efi", "--efi-directory=/boot/efi", "--boot-directory=/boot", "--bootloader-id=metal-debian", "--removable"},
					Output:   "",
					ExitCode: 0,
				},
				{
...
`,
		},
		{
			...
			execMocks: []test.FakeExecParams{
				...
				{
					WantCmd:  []string{"efibootmgr", "-c", "-d", "/dev/sda1", "-p1", "-l", "\\\\EFI\\\\metal-debian\\\\grubx64.efi", "-L", "metal-debian"},
					Output:   "",
					ExitCode: 0,
				},
				{
					WantCmd:  []string{"efibootmgr", "-c", "-d", "/dev/sdb1", "-p1", "-l", "\\\\EFI\\\\metal-debian\\\\grubx64.efi", "-L", "metal-debian"},
					Output:   "",
					ExitCode: 0,
				},
				{
					WantCmd:  []string{"grub-install", "--target=x86_64-efi", "--efi-directory=/boot/efi", "--boot-directory=/boot", "--bootloader-id=metal-debian", "--removable", "--no-nvram"},
					Output:   "",
					ExitCode: 0,
				},
				{
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			content, err := fs.ReadFile(oscommon.DefaultGrubPath)
