...
func Test_os_WriteNTPConf(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fsMocks: func(fs *afero.Afero) {
				require.NoError(t, fs.WriteFile(centos.ChronyConfigPath, []byte(""), 0644))
			},
			allocation: &apiv2.MachineAllocation{
...
`,
		...
		},
		{
			...
			want:    "",
			wantErr: fmt.Errorf("centos as firewall is currently not supported"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			content, err := fs.ReadFile(centos.ChronyConfigPath)
