...
func Install(ctx context.Context, log *slog.Logger, config *v1.InstallerConfig) error {
	start := time.Now()
	fs := afero.OsFs{}
...
func (i *installer) do() error {
	...
	err = i.writeHosts()
	if err != nil {
		i.log.Warn("writing hosts file failed", "error", err)
		return err
	}
...
func (i *installer) writeHosts() error {
	// FIXME: figure out how to get the private primary ip
	return afero.WriteFile(i.fs, "/etc/hosts", []byte(fmt.Sprintf(`# this file was auto generated by the os-installer
127.0.0.1        localhost
%s     %s
`, i.config.PrivateIP, i.config.Hostname)), 0644)
}
...
func (i *installer) writeNTPConf() error {
	...
	for _, srv := range i.config.NTPServers {
		servers = append(servers, *srv.Address)
	}

	switch i.config.Role {
	case models.V1MachineAllocationRoleFirewall:
		_, err := chrony.WriteSystemdUnit(i.ctx, &chrony.Config{
			Log:    i.log,
			Reload: false,
			Enable: true,
		}, &chrony.TemplateData{
			NTPServers: servers,
		}, "TODO") // FIXME: default vrf
